...
const FC_MARKER_START = '<!-- YQ-FAST-CONTEXT-START -->'
const FC_MARKER_END = '<!-- YQ-FAST-CONTEXT-END -->'

/**
 * Write fast-context search guidance to:
 * 1. ~/.claude/rules/yq-fast-context.md (Claude Code — auto-loaded via rules/)
...
export async function writeFastContextPrompt(auxiliaryMode = false): Promise<void> {
  ...
  await fs.writeFile(join(rulesDir, 'yq-fast-context.md'), promptContent, 'utf-8')
...
export async function removeFastContextPrompt(): Promise<void> {
  ...
  const rulePath = join(homedir(), '.claude', 'rules', 'yq-fast-context.md')
