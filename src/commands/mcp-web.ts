...
function getClientIdFromEnv(): McpClientId | null {
  ...
  return clientId === 'claude' || clientId === 'codex' || clientId === 'gemini' || clientId === 'cursor' || clientId === 'kiro' ? clientId : null
}
...
function renderMcpAppHtml(): string {
  ...
  *{box-sizing:border-box}html,body{height:100%}body{margin:0;background:radial-gradient(circle at top,#eef4ff 0,#f5f7fb 22%,#f5f7fb 100%);color:var(--text)}
  ...
  .toast{pointer-events:auto;min-width:280px;max-width:420px;padding:14px 18px;border-radius:14px;font-size:14px;line-height:1.5;color:#fff;background:#344054;box-shadow:0 8px 30px rgba(15,23,42,.18);transform:translateX(120%);opacity:0;transition:transform .3s ease,opacity .3s ease}
  .toast.show{transform:translateX(0);opacity:1}
  .toast.success{background:#067647}.toast.error{background:#b42318}.toast.info{background:#2563eb}
  ...
  serverIdInput.oninput=()=>{state.modalDirty=true;};configJson.oninput=()=>{state.modalDirty=true;};
  document.addEventListener('keydown',event=>{const tag=(event.target.tagName||'').toLowerCase();const isInput=tag==='input'||tag==='textarea'||tag==='select';if(event.key==='Escape'&&overlay.classList.contains('open')){event.preventDefault();closeModal();return;}if(overlay.classList.contains('open')){if((event.ctrlKey||event.metaKey)&&event.key==='s'){event.preventDefault();saveServer();}return;}if(isInput)return;if(event.key==='/'&&!event.ctrlKey&&!event.metaKey){event.preventDefault();searchInput.focus();return;}if((event.ctrlKey||event.metaKey)&&event.key==='n'){event.preventDefault();openModal();return;}});
  ...
  async function loadServers(){statusEl.textContent='正在读取 '+state.clientId+' 配置...';try{const payload=await fetchJson('/api/servers?client='+encodeURIComponent(state.clientId));state.servers=payload.servers;clientName.textContent=payload.client.label;clientPath.textContent=payload.client.configPath;searchInput.value='';state.search='';renderList();updateSummary();statusEl.textContent='配置已同步';}catch(error){statusEl.textContent='读取配置失败';showToast(error instanceof Error?error.message:'读取配置失败','error');}}
  ...
  function openModal(serverId){state.editingId=serverId||null;state.modalDirty=false;state.modalTrigger=document.activeElement;const server=state.servers.find(item=>item.id===serverId);modalTitle.textContent=server?'编辑 MCP 服务 · '+server.id:'添加 MCP 服务 · '+state.clientId.toUpperCase();modalSub.textContent=server?'继续微调 JSON，保存前会先创建配置备份。':'先选模板再改 JSON 会更快。';serverIdInput.value=server?server.id:'';serverIdInput.disabled=Boolean(server);configJson.value=JSON.stringify(server?server.config:{type:'stdio',command:'npx',args:['-y','your-package']},null,2);overlay.classList.add('open');setTimeout(()=>{(server?configJson:serverIdInput).focus();},50);}
  function closeModal(force){if(!force&&state.modalDirty&&!window.confirm('当前修改尚未保存，确认放弃？'))return;overlay.classList.remove('open');state.editingId=null;state.modalDirty=false;serverIdInput.disabled=false;if(state.modalTrigger){try{state.modalTrigger.focus();}catch{}state.modalTrigger=null;}}
  async function saveServer(){const btn=document.getElementById('saveBtn');setBusy(btn,true,'保存中...');try{let config;try{config=JSON.parse(configJson.value);}catch(parseErr){const msg=parseErr instanceof Error?parseErr.message:'JSON 格式错误';showToast('JSON 解析失败：'+msg,'error');return;}const payload={clientId:state.clientId,serverId:serverIdInput.value.trim(),config,enabled:true};if(!payload.serverId){showToast('服务器 ID 不能为空','error');return;}await fetchJson('/api/server',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal(true);await loadServers();showToast('已保存 '+payload.serverId,'success');}catch(error){showToast(error instanceof Error?error.message:'保存失败','error');}finally{setBusy(btn,false);}}
  async function toggleServer(serverId,enabled){try{await fetchJson('/api/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({clientId:state.clientId,serverId,enabled})});await loadServers();showToast(serverId+(enabled?' 已启用':' 已停用'),'success');}catch(error){showToast(error instanceof Error?error.message:'切换失败','error');await loadServers();}}
  async function removeServer(serverId){if(!window.confirm('确认删除 '+serverId+' 吗？'))return;try{await fetchJson('/api/server?client='+encodeURIComponent(state.clientId)+'&id='+encodeURIComponent(serverId),{method:'DELETE'});await loadServers();showToast('已删除 '+serverId,'success');}catch(error){showToast(error instanceof Error?error.message:'删除失败','error');}}
  ...
  async function installSmitheryCli(){const confirmed=window.confirm('将执行 npm install -g @smithery/cli@latest，是否继续？');if(!confirmed)return;const btn=document.getElementById('smitheryInstallBtn');setBusy(btn,true,'安装中...');smitheryStatusText.textContent='正在安装 / 更新 Smithery CLI，请稍候...';try{const payload=await fetchJson('/api/smithery/install',{method:'POST'});state.smithery.status=payload.status;smitheryVersionPill.textContent=payload.status.installed&&payload.status.version?'CLI v'+payload.status.version:payload.status.installed?'CLI 已安装':'CLI 未安装';smitheryVersionPill.className='pill'+(payload.status.installed?' ok':'');smitheryStatusText.textContent=payload.status.installed?'Smithery CLI 已安装，可直接使用 smithery --help 或下方搜索。':'Smithery CLI 安装命令已执行，但当前仍未检测到 smithery，请检查 PATH 或重新打开终端。';showToast('Smithery CLI 安装完成','success');}catch(error){smitheryStatusText.textContent=error instanceof Error?error.message:'Smithery CLI 安装失败';showToast(error instanceof Error?error.message:'Smithery CLI 安装失败','error');}finally{setBusy(btn,false);}}
  async function searchSmithery(){const query=String(smitherySearchInput.value||'').trim();state.smithery.query=query;if(!query){state.smithery.results=[];renderSmitheryResults();smitherySearchStatus.textContent='输入关键字后开始搜索';return;}const btn=document.getElementById('smitherySearchBtn');setBusy(btn,true,'搜索中...');smitherySearchStatus.textContent='正在通过 smithery mcp search 检索...';try{const payload=await fetchJson('/api/smithery/search?query='+encodeURIComponent(query));state.smithery.results=Array.isArray(payload.servers)?payload.servers:[];renderSmitheryResults();smitherySearchStatus.textContent=state.smithery.results.length?'共找到 '+state.smithery.results.length+' 个结果':'未找到匹配结果';}catch(error){state.smithery.results=[];renderSmitheryResults();smitherySearchStatus.textContent=error instanceof Error?error.message:'Smithery 搜索失败';showToast(error instanceof Error?error.message:'Smithery 搜索失败','error');}finally{setBusy(btn,false);}}
  ...
  async function closeService(){if(!window.confirm('确认关闭本地 MCP 配置页服务？'))return;try{const response=await fetch('/api/close',{method:'POST'});if(!response.ok)throw new Error('关闭请求失败');document.body.innerHTML='<div style="display:flex;min-height:100vh;align-items:center;justify-content:center;font:16px/1.6 Segoe UI,system-ui,sans-serif;color:#475467;background:#f5f7fb;">本地 MCP 配置页已关闭，现在可以关闭这个页面了。</div>';setTimeout(()=>window.close(),300);}catch(error){showToast(error instanceof Error?error.message:'关闭服务失败','error');}}
  ...
  function showToast(message,type='info'){const container=document.getElementById('toastContainer');const toast=document.createElement('div');toast.className='toast '+type;toast.textContent=message;container.appendChild(toast);requestAnimationFrame(()=>{requestAnimationFrame(()=>toast.classList.add('show'));});setTimeout(()=>{toast.classList.remove('show');setTimeout(()=>toast.remove(),300);},3500);}
  function setBusy(btn,busy,label){if(!btn)return;if(busy){btn.disabled=true;btn._origHtml=btn.innerHTML;btn.innerHTML='<span class="spinner"></span>'+(label||'处理中...');}else{btn.disabled=false;if(btn._origHtml)btn.innerHTML=btn._origHtml;}}
  </script></body></html>`
}
