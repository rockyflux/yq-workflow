...
async function writeGrokPromptToRules(): Promise<void> {
  ...
  const rulePath = join(rulesDir, 'yq-grok-search.md')

  // Clean up legacy CLAUDE.md injection if present
  const claudeMdPath = join(homedir(), '.claude', 'CLAUDE.md')
  if (await fs.pathExists(claudeMdPath)) {
    const content = await fs.readFile(claudeMdPath, 'utf-8')
    if (content.includes('YQ-GROK-SEARCH-PROMPT')) {
      const cleaned = content.replace(/\n*<!-- YQ-GROK-SEARCH-PROMPT-START -->[\s\S]*?<!-- YQ-GROK-SEARCH-PROMPT-END -->\n*/g, '')
...
async function handleGrokSearch(): Promise<void> {
  ...
  if (result.success) {
    ...
    console.log(ansis.green('✓ 全局搜索提示词已写入 ~/.claude/rules/yq-grok-search.md'))
    await syncMcpMirrors()
    console.log(ansis.gray('  重启 Claude Code CLI 使配置生效'))
  }
  else {
