# YQ Quality Gates — Auto-trigger Rules

When working in a project, automatically invoke the corresponding quality gate skills based on the scenario below. These skills are installed at `~/.claude/skills/yq/` and can be called directly.

**IMPORTANT**: Skill names do NOT have a `yq:` prefix. Use exactly: `verify-security`, `verify-quality`, `verify-change`, `verify-module`, `gen-docs`. Do NOT call `yq:verify-security` — that will fail with "Unknown skill".
...
When a task involves 3+ independent files/modules or 2+ parallel workflows, refer to the multi-agent orchestration skill at `~/.claude/skills/yq/orchestration/multi-agent/SKILL.md` for:
