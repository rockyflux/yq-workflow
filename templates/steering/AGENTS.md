...
- 默认文档根目录：`~/Obsidian/Codex`。
- 默认按“当前仓库所在文件夹层级”在该根目录下建立子目录后再落文档；例如仓库 `~/Documents/Code/stock/trading_system` 的默认文档目录为 `~/Obsidian/Codex/stock/trading_system`。
...
- 技能存放位置：`~/.agents/skills/`（个人）与 `.codex/skills/`（项目共享，可选）。
