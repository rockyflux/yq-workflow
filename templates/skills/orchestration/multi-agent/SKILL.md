...
`/yq:team` 命令在蚁群基础角色之上，增加 3 个大厂级专业角色，对应 Agent Teams 真实 teammates：
...
**推荐入口**：`/yq:team <需求描述>` — 一键跑完 8 阶段。
**分步入口**（兼容）：`/yq:team-research` → `/yq:team-plan` → `/yq:team-exec` → `/yq:team-review`
