...
主菜单顶部的已安装版本检测会兼容 `v1.1.0` / `1.1.0` 这类历史版本格式，避免把同版本误报成“当前启动版本较旧”；若当前实际运行的 CLI 版本确实更旧，主菜单与更新页会明确提示“当前运行的是较旧 CLI 入口”，并尽量识别这是 `npx` 缓存、全局安装还是其他来源，再建议改用 `npx --yes yq-workflow@latest` 或升级全局安装，避免误以为只是工作流文件没更新。
