...
- 安装并管理整套工作流
- 安装全局提示词文件到 `~/.claude/CLAUDE.md`、`~/.codex/AGENTS.md`、`~/.cursor/rules/guidelines.mdc`、`~/.kiro/steering/kiro.md`
...
进入 `配置 MCP`，用本地网页分别管理 Claude、Codex、Gemini、Cursor、Kiro 的 MCP 配置。

### 3. 想整理 Skills

进入 `配置 Skills`，查看本地技能目录、安装新 Skills，或者跳转到 `Skills Manage` 桌面软件项目页。

### 4. 想调整提示词

进入 `提示词配置`，统一编辑常用提示词文件；保存前会先确认，并自动保留备份。你也可以在这里继续微调“什么时候该把经验、方案、排障记录写到 Obsidian”的规则。
