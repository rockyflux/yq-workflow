...
YQ 是一个面向 Claude Code 的单模型工作流工具包。
...
目标很直接：让 Claude Code 拥有更完整的工作流、命令入口和协作约束，而不是再套一层黑盒编排。

## 需要什么

- **Node.js 20+**：`ora@9.x` 要求 Node 20，Node 18 会直接报语法错误
- **Claude Code CLI**：YQ 的命令包和技能默认面向它安装
...
- 主菜单：安装 / 更新工作流、配置 MCP、配置 API、选择输出风格、安装 Claude Code / Codex、检查工具更新
